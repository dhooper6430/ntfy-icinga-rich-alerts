...
Acknowledge / Downtime actions are **HMAC-signed** (over every field the relay passes to Icinga,
with an expiring timestamp, so captured messages can't be replayed or altered) and verified by the
relay before it touches Icinga, and the Icinga API is reached through a **scoped** ApiUser limited
to acknowledge + schedule downtime — phones never hold Icinga credentials. Everything runs on infrastructure you control; the
