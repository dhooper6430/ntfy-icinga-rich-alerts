#!/usr/bin/env python3
"""Ack relay — make the Acknowledge / Downtime buttons work with NO inbound exposure.

The notification buttons publish a small HMAC-signed message to an ntfy *ack topic*. This service
SUBSCRIBES to that topic — an outbound connection, exactly like the dispatcher's publish — and for
each valid message calls the local Icinga2 API to acknowledge the problem or schedule downtime.

Because every connection is outbound, the buttons work even when your Icinga has no public IP, no
port-forward and no tunnel: point the dispatcher and the phone at a reachable ntfy (your own or
the public ntfy.sh) and run this next to the dispatcher.

Config is read from the dispatcher's config.yml (see config.example.yml: the `relay:` section,
plus ntfy.base_url, actions.ack_topic and actions.shared_secret). Run it as a long-lived service —
see relay.service.example for a systemd unit.
"""
from __future__ import annotations

import json
import logging
import os
import sys
import time

import requests

import action_tokens
from config import load_config

log = logging.getLogger("relay")

DEFAULT_CONFIG = os.environ.get(
    "DISPATCHER_CONFIG",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "config.yml"),
)


class Cursor:
    """Persists the id of the last processed ack-topic message so reconnects resume with
    `since=<id>` and an action tapped during an outage isn't silently lost. Fails open: if the
    state file is unwritable the relay still runs, it just can't replay across a restart."""

    def __init__(self, path: str) -> None:
        self.path = path
        self.last_id = ""
        self._warned = False
        try:
            with open(path, encoding="utf-8") as fh:
                self.last_id = fh.read().strip()
        except OSError:
            pass

    def save(self, msg_id: str) -> None:
        self.last_id = msg_id
        try:
            tmp = f"{self.path}.tmp"
            with open(tmp, "w", encoding="utf-8") as fh:
                fh.write(msg_id)
            os.replace(tmp, self.path)
        except OSError as exc:
            if not self._warned:
                log.warning("cannot persist relay cursor to %s (%s) — "
                            "actions arriving during a restart may be lost", self.path, exc)
                self._warned = True


class IcingaClient:
    """Minimal Icinga2 /v1/actions client (self-contained)."""

    def __init__(self, rcfg: dict) -> None:
        self.url = str(rcfg.get("icinga_api_url", "https://localhost:5665")).rstrip("/")
        self.user = str(rcfg.get("icinga_api_user", ""))
        self.password = str(rcfg.get("icinga_api_password", ""))
        self.ca = rcfg.get("icinga_api_ca", "")
        self.insecure = bool(rcfg.get("icinga_api_insecure", False))

    @property
    def _verify_tls(self):
        if self.insecure:
            return False
        return self.ca or True

    def _filter(self, host: str, service: str) -> dict:
        # filter_vars keeps host/service out of the filter string (no injection).
        if service:
            return {"type": "Service",
                    "filter": "host.name==hostname && service.name==servicename",
                    "filter_vars": {"hostname": host, "servicename": service}}
        return {"type": "Host", "filter": "host.name==hostname", "filter_vars": {"hostname": host}}

    def _action(self, action: str, payload: dict):
        return requests.post(f"{self.url}/v1/actions/{action}", json=payload,
                             auth=(self.user, self.password),
                             headers={"Accept": "application/json"},
                             verify=self._verify_tls, timeout=10)

    def acknowledge(self, host, service, author, comment):
        return self._action("acknowledge-problem", {
            **self._filter(host, service), "author": author, "comment": comment,
            "notify": True, "sticky": False})

    def downtime(self, host, service, author, comment, hours):
        try:
            seconds = int(float(hours) * 3600)
        except (TypeError, ValueError):
            seconds = 3600
        now = int(time.time())
        return self._action("schedule-downtime", {
            **self._filter(host, service), "author": author, "comment": comment,
            "start_time": now, "end_time": now + seconds, "duration": seconds, "fixed": True})


def handle(message: str, secret: str, icinga: IcingaClient, default_comment: str,
           token_ttl: int = action_tokens.DEFAULT_TTL) -> None:
    try:
        data = json.loads(message)
    except (ValueError, TypeError):
        return  # not one of our action messages
    if not isinstance(data, dict):
        return  # valid JSON but not an object (anyone can post to the topic) — not ours
    fields = {k: data.get(k) or "" for k in ("action", "host", "service", "author", "comment")}
    if any(not isinstance(v, str) for v in fields.values()):
        log.warning("ignoring malformed action message (non-string fields)")
        return
    action, host, service = fields["action"].strip(), fields["host"].strip(), fields["service"].strip()
    if not host or action not in ("ack", "downtime"):
        log.warning("ignoring malformed action message (action=%r host=%r)", action, host)
        return
    # The token must cover every field we pass to Icinga — verify against the RAW values as
    # signed by notify.py; normalisation/defaults only after the signature checks out.
    hours = data.get("hours", "")
    if not action_tokens.verify(secret, data.get("token"), time.time(), token_ttl,
                                action=action, host=host, service=service,
                                author=fields["author"], comment=fields["comment"],
                                hours=hours, ts=data.get("ts")):
        log.warning("REJECTED %s %s/%s — bad or expired token", action, host, service)
        return
    author = fields["author"].strip() or "ntfy"
    comment = fields["comment"].strip() or default_comment
    target = f"{host}{('/' + service) if service else ''}"
    try:
        if action == "ack":
            resp = icinga.acknowledge(host, service, author, comment)
        else:
            resp = icinga.downtime(host, service, author, comment, hours or 1)
        if resp.status_code == 409:
            # Already acknowledged / already in downtime — idempotent success (double-tap).
            log.info("%s %s by %s -> already done (http=409)", action, target, author)
        elif resp.ok:
            log.info("%s %s by %s -> http=%s", action, target, author, resp.status_code)
        else:
            log.error("%s %s by %s REJECTED by Icinga: http=%s %s",
                      action, target, author, resp.status_code, (resp.text or "")[:200])
    except requests.RequestException as exc:
        log.error("%s %s failed to reach Icinga: %s", action, target, exc)


def subscribe_loop(cfg: dict, config_path: str = DEFAULT_CONFIG) -> None:
    base = cfg["ntfy"]["base_url"].rstrip("/")
    topic = cfg["actions"]["ack_topic"]
    rcfg = cfg.get("relay", {})
    secret = cfg["actions"]["shared_secret"]
    token_ttl = int(cfg["actions"].get("token_ttl", action_tokens.DEFAULT_TTL))
    default_comment = rcfg.get("default_comment", "Actioned from ntfy")
    icinga = IcingaClient(rcfg)
    cursor = Cursor(rcfg.get("state_file")
                    or os.path.join(os.path.dirname(os.path.abspath(config_path)), "relay-cursor"))
    url = f"{base}/{topic}/json"
    headers = {}
    if rcfg.get("ack_read_token"):
        headers["Authorization"] = f"Bearer {rcfg['ack_read_token']}"
    log.info("relay subscribing to %s (icinga=%s, cursor=%s)", url, icinga.url,
             cursor.last_id or "live")
    backoff = 1
    while True:
        try:
            # since=<last id> replays anything published while we were down/reconnecting, so a
            # tapped button is never lost to an outage. Replays are safe: expired tokens are
            # rejected and a re-ack is a 409 (idempotent).
            params = {"since": cursor.last_id} if cursor.last_id else {}
            with requests.get(url, headers=headers, params=params,
                              stream=True, timeout=(10, 75)) as r:
                r.raise_for_status()
                backoff = 1
                for line in r.iter_lines(decode_unicode=True):
                    if not line:
                        continue
                    try:
                        evt = json.loads(line)
                    except ValueError:
                        continue
                    if not isinstance(evt, dict) or evt.get("event") != "message":
                        continue
                    # One bad message must never kill the subscriber (the topic is publicly
                    # writable in the ntfy.sh deployment mode) — log it and move on.
                    try:
                        handle(evt.get("message", ""), secret, icinga, default_comment, token_ttl)
                    except Exception:
                        log.exception("error handling action message id=%s — skipped", evt.get("id"))
                    if evt.get("id"):
                        cursor.save(evt["id"])
        except requests.RequestException as exc:
            log.warning("subscription dropped (%s); reconnecting in %ss", exc, backoff)
            time.sleep(backoff)
            backoff = min(backoff * 2, 60)


def main() -> int:
    logging.basicConfig(level=logging.INFO, format="%(asctime)s %(levelname)s relay: %(message)s")
    config_path = os.environ.get("DISPATCHER_CONFIG", DEFAULT_CONFIG)
    cfg = load_config(config_path)
    subscribe_loop(cfg, config_path)
    return 0


if __name__ == "__main__":
    sys.exit(main())
