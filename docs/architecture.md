...
6. Tapping **Acknowledge** or **Downtime** publishes an HMAC-signed action message (a token over
   every field the relay passes to Icinga — action, host, service, author, comment, hours — plus
   a timestamp, expiring after `actions.token_ttl`) to an ntfy **ack topic**. `relay.py`
   subscribes to that topic **outbound** (exactly like the dispatcher publishes), verifies the
   token, and calls the local Icinga API as a **scoped** ApiUser (only `acknowledge-problem` +
   `schedule-downtime`).
...
- The Icinga API credentials never touch the device. The phone only ever holds a short, expiring
  HMAC token scoped to one exact action (host, service, author, comment, duration), embedded in
  the button — a captured message can't be replayed later or with altered fields.
...
- The relay collapses Icinga's "already acknowledged / already in downtime" (HTTP 409) into an
  idempotent success, so a double-tap doesn't surface an error on the phone; any other non-2xx
  from Icinga (bad ApiUser credentials, unknown host) is logged at ERROR so it can't fail silently.
- The relay resumes its ntfy subscription from the last processed message id (`relay.state_file`),
  so an action tapped while it was down/reconnecting is replayed, not lost.
